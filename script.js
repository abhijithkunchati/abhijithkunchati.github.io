  ...
  if (e.key === 'Tab'){
    if (y === -1){
      ...
      if(result.includes("cd a")){
        input.value = "cd Abhijith\\ Kunchati/";
      } 
  ...
  if(e.key === 'Enter'){
    if(result === ""){
      ...
      if (y === -1){
        ...
        if(result === "abhijith kunchati" || result === "cd abhijith kunchati" || result === "cd abhijith\\ kunchati/" || result === "abhijith\\ kunchati/" || result === "cd"){
          y ++;
      ...
      if (y === -1){
        terminal = document.getElementById("terminal");
        terminal.insertAdjacentHTML("beforeend", `<p class="commande" id="answer${i}">${window.rawdata.name}:~$:</p><input type="text" autofocus class="input themeinput" id="input${i}" tabindex="-1">`);
      } else if (y === 1){
        terminal = document.getElementById("terminal");
        terminal.insertAdjacentHTML("beforeend", `<p class="commande" id="answer${i}">${window.rawdata.name}:~/portfolio/themes$:</p><input type="text" autofocus class="input themeinput" id="input${i}" tabindex="-1">`);
      } else {
        terminal = document.getElementById("terminal");
        terminal.insertAdjacentHTML("beforeend", `<p class="commande" id="answer${i}">${window.rawdata.name}:~/portfolio$</p><input type="text" autofocus class="input" id="input${i}" tabindex="-1">`);
      }
      document.getElementById(`input${i}`).focus();
